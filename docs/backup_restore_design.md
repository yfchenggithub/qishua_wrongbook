...
- App 启动、切到前台、切到后台或运行期间跨过本地零点时，都会立即检查并生成当天备份，不等待 Android 后台调度。
- 备份开始后不会因为 App 在前台与后台之间切换而取消；同一时刻的重复触发会复用正在执行的任务。
- Android 后台任务仍按小时获得补充检查机会；无论任务回调执行时 App 是否在前台，都直接完成当天备份。当天已有备份时只复用现有文件，保证每天最多生成 1 份。
