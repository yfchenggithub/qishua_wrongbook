...
const PRIVACY_POLICY_SECTIONS: LegalDocumentSection[] = [
  ...
  },
  {
    title: '5. 运行日志、反馈与联系',
    content:
      '运行日志默认仅保存在当前 App 运行环境中，可能包含 App 与 build 版本、系统版本、手机品牌与型号、CPU 与硬件类别、设备和 App 内存占用、可用存储、电池电量与温度、节电和温控状态、App 前后台状态、语言时区、显示环境，以及仅在本次运行期间有效的匿名会话标识，用于定位兼容性和运行问题；不会记录姓名、手机号、精确定位、网络详情、错题图片内容或持久设备标识。只有当你主动导出并分享日志时，这些日志才会离开设备。当你通过邮箱反馈问题时，我们可能会收到你的邮箱地址、问题描述、截图或你主动提供的信息，这些信息仅用于问题定位、用户支持和产品改进。',
  },
  {
    title: '6. 第三方服务',
    content:
